...
func AttachmentPresignedURL(att *model.AttachmentModel) string {
	if att == nil {
		return ""
	}
	manager := GetStorageManager()
	...
	return manager.PresignedURL(context.Background(), convertModelToBackend(att.StorageType), att.ObjectKey)
}
